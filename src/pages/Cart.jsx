...
export default function Cart({ cart, onUpdateQty, onRemove }) {
  ...
  const handleWhatsAppOrder = () => {
    ...
    message += `\n${lang === "fr" ? "Merci de confirmer disponibilité et livraison." : lang === "ar" ? "يرجى تأكيد التوفر والتوصيل." : "Please confirm availability and delivery."}`;
  ...
  if (cart.length === 0) {
    return (
      <MarbleBackground>
        <Section>
          <div className="text-center py-16 max-w-md mx-auto bg-white/80 backdrop-blur border border-white/50 p-8 shadow-[0_10px_40px_rgba(201,168,106,0.12)]">
            <div className="w-20 h-20 mx-auto border border-border flex items-center justify-center text-secondary mb-6 bg-white">
