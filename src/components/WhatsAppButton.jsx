export default function WhatsAppButton() {
  ...
  return (
    <div className="fixed bottom-24 md:bottom-5 left-5 z-[60] flex flex-col items-center gap-1.5">
      <a
        href={`https://wa.me/${number}`}
        target="_blank"
        rel="noopener noreferrer"
        aria-label="Chat on WhatsApp"
        className="w-[56px] h-[56px] bg-[#25D366] rounded-full flex items-center justify-center shadow-[0_6px_20px_rgba(0,0,0,0.2)] hover:scale-105 transition-transform duration-300"
      >
      ...
      {/* Invisible spacer matching the Commander label height so both circles sit exactly level */}
      <span aria-hidden="true" className="invisible select-none text-[11px] font-medium tracking-wide px-3 py-1 border border-transparent whitespace-nowrap">
        {"\u00A0"}
      </span>
    </div>
  );
