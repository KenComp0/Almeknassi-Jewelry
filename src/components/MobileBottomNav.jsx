...
export default function MobileBottomNav() {
  ...
  const orderLabel = lang === "fr" ? "Commander" : lang === "ar" ? "اطلبي" : "Order";
    ...
    >
      <div className="flex items-end justify-around px-6 pt-2 pb-3 relative">
        ...
        <Link to="/" className="flex flex-col items-center gap-1 py-1 text-white/80 hover:text-[#C9A86A] min-w-[64px]">
          ...
          <span className="text-[10px] tracking-widest uppercase">{t("nav.home")}</span>
        </Link>

        {/* Buy - center, bigger than sides but smaller than floating pill, raised */}
        <button type="button" onClick={handleBuy} aria-label="Commander" className="flex flex-col items-center -mt-7">
          ...
          <span className="text-[10px] tracking-widest uppercase text-[#C9A86A] mt-1">{orderLabel}</span>
        </button>
              ...
              >
                {LANGS.map((l) => (
                  <button
                    key={l.code}
                    type="button"
                    onClick={(e) => handleSelect(e, l.code)}
